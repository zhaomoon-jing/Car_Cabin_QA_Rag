...
<<<<<<< HEAD
# 品牌匹配统一复用 retriever/bm25_retriever.py 的 BRAND_LIST_SORTED（按长度降序），
# 新增品牌只需在 BRAND_LIST 一处维护，这里不再单独维护匹配表。
from retriever.bm25_retriever import BRAND_LIST_SORTED

# 仅用于界面提示语展示的常用品牌（匹配逻辑走 BRAND_LIST_SORTED，此处可随意精简）
SUPPORT_BRANDS = [
    "比亚迪", "宝马", "蔚来", "理想", "特斯拉", "小鹏", "问界", "极氪"
]

def extract_brand_from_query(query: str):
    """
    从用户问句自动提取车辆品牌（复用检索侧品牌表，保证一致）
    例："宝马座椅加热怎么开" → "宝马"；"故障码P0123" → None
    """
...
=======
>>>>>>> parent of d765a53 (release version)
