...
构建chroma数据库
1.执行mineru  解析pdf。
2.根据解析文件raw_output中 content_list.json 切块后创建数据库
  python data_process/build_chunks.py
  python data_process/build_chunk_chorma.py
  输出向量到car_chroma_db

3. 验证检索正常
   python retriever/bm25_retriever.py
   python retriever/dense_retriever.py
（可选）训练意图分类：python intent_cls/train_intent.py 训练完成后自动生成 intent_finetrne 权重文件夹，后续推理时候infer_intent.py 可直接加载推理
（可选）LoRA 领域微调：python llm_infer/train_lora.py (提前准备好QA数据)
4. python main.py
...
接口访问地址：
1、健康检测：http://车机IP:7890/health
    curl http://127.0.0.1:7890/health
2、文本问答 POST 接口：http://IP:7890/chat_text，传参 query="如何打开座椅加热"
   curl -X POST -G "http://127.0.0.1:7890/chat_text"   --data-urlencode "query=故障码U0001是什么"
   curl -X POST -G "http://127.0.0.1:7890/chat_text"   --data-urlencode "query=保养周期是多久"
音频上传接口：http://IP:7890/chat_audio，form-data 上传 wav 音频
自动接口文档：http://IP:7890/docs，可在线调试所有接口

杀死进程pkill -f car_api; sleep 2; nvidia-smi


# m4a → wav（16kHz 单声道，跟 FunASR 要求一致）
ffmpeg -i 你的录音.m4a -ar 16000 -ac 1 你的录音.wav

# 验证是不是真 wav
file 你的录音.wav
# 应该输出: Audio file with ID3 tag 或 RIFF (little-endian) data, WAVE audio...
